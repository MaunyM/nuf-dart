...
export default function PartComponent(props: PartProps) {
  ...
  const start: (r: number) => Coord = (r: number) => ({
    x: cos_start * r,
    y: sin_start * r,
  });
  const end: (r: number) => Coord = (r: number) => ({
    x: cos_end * r,
    y: sin_end * r,
  });
  ...
  return (
    <g>
      <path
        className="part"
        fill={props.joueur?`url(#grad-${props.joueur?.nom})`:props.gameType===Game_Type.MONSTER?'rgb(200, 200,200)':''}
